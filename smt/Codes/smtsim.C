...
void smtsim()
{
  ...
  TFile* ff = new TFile("/home/jcrosby/WorkPlace/Histograms/Root_Files/mu_pTrel_rebin.root");
  TFile* ff_mc = new TFile("/home/jcrosby/WorkPlace/Histograms/Root_Files/mu_pTrel_rebin.root");
  ...
  TH1* tot =  new TH1D("","",nbins,0,nbins);

  //Stacked Histogram Creation
      
  TH1* h[nfl];
  THStack* ths = new THStack("","pTrel_PreFit");
  for(int flav = 0; flav<nfl; ++flav){
    TString sh = "jet_mu_pTrel_"; sh += chfl[flav];
    TString final_name =  sh;
    //cout << final_name <<endl;
    TH1* mc_raw = (TH1*)ff_mc->Get(final_name);
    sh += "_pr";
    h[flav] = new TH1D(sh,"",nbins,0,nbins);
  ...
  ths->GetYaxis()->SetTitleSize(20);
  ths->GetYaxis()->SetTitleFont(43);
  ths->GetYaxis()->SetTitleOffset(1);
  ths->SetMinimum(1e2);
  hd->SetMarkerStyle(8);
  ...
  scf->GetXaxis()->SetTitleSize(20);
  scf->GetXaxis()->SetTitleFont(43);
  scf->GetXaxis()->SetTitleOffset(2.5);
  ...
  if (save_plots) {
    TString sp = "jet_mu_pTrel_prefit.png";
    c1->Print(sp);
  }
  ...
  TH1* totf = new TH1D("","",nbins,0,nbins);
  ...
  TH1* hs[nfl];
  THStack* thss = new THStack("",  "pTrel_PostFit");
  ...
  thss->GetYaxis()->SetTitleSize(20);
  thss->GetYaxis()->SetTitleFont(43);
  thss->GetYaxis()->SetTitleOffset(1);
  thss->SetMinimum(1e2);
  hd->SetMarkerStyle(8);
  hd->SetMarkerSize(1.2);
  hd->Draw("esame");
  ...
  // X axis ratio plot settings
  scff->GetXaxis()->SetTitleSize(20);
  scff->GetXaxis()->SetTitleFont(43);
  scff->GetXaxis()->SetTitleOffset(2.5);
  scff->GetXaxis()->SetLabelFont(43); // Absolute font size in pixel 
  scff->GetXaxis()->SetLabelSize(15);
       
  scff->Draw("e same");
      
  TLine *line2 = new TLine(0,1,nbins,1);
  line2->SetLineColor(kBlack);
  line2->Draw();
      
  if (save_plots) {
    TString sp = "jet_mu_pTrel_postfit.png";
    c2->Print(sp);
  }
